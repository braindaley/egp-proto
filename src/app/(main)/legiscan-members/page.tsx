    ...
    <div className="bg-secondary/30 flex-1">
      <div className="container mx-auto px-4 py-8 md:py-12 max-w-2xl">
        <header className="text-center mb-12">
          <p className="text-lg text-muted-foreground font-medium mb-1">State Legislators</p>
          <h1 className="font-headline text-4xl md:text-5xl font-bold text-primary mb-2">
            Explore State Legislators
          </h1>
          <p className="text-lg text-muted-foreground">
            Select a state to view its current senators and representatives.
          </p>
        </header>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {states.map((state) => (
            <Link
              href={`/legiscan-members/${state.abbr.toLowerCase()}`}
              key={state.abbr}
