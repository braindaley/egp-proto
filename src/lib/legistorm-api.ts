...
export async function getLocalOfficials(
  ...
  try {
    // LegiStorm localOfficials API requires date range, not state filter
    // We fetch all recent data and filter by state client-side
    const params = new URLSearchParams({
    ...
    // Filter by state since API doesn't support state parameter
    const allOfficials = Array.isArray(data) ? data : [];
    const filteredOfficials = allOfficials.filter(
      (official: any) => official.state_id === stateAbbr.toUpperCase()
    );
