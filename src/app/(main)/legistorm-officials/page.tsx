    ...
    <Card className="hover:shadow-lg transition-shadow">
      ...
      <CardContent>
        ...
        <div className="space-y-2">
          ...
          {officeHolder.totalYearsInOffice > 0 && (
...
export default function LegistormOfficialsPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/legistorm-officials?${params.toString()}`);
      ...
      if (!response.ok) {
        ...
        if (data.count === 0) {
          setError('No elected officials found for this location. The LegiStorm API may not have data for this area yet.');
        }
      }
    } catch (err) {
